...
const DEMO_WALLET = "GBXYZ...INVST9012"

export function InvestorProfile() {
  const { investorUser, setInvestorUser, profileImage, setProfileImage, setScreen, resetApp } = useApp()
  const { t } = useTranslation()
  const [editingEmail, setEditingEmail] = useState(false)
  const [emailDraft, setEmailDraft] = useState(investorUser.email)
  const [copied, setCopied] = useState(false)
  const [notifPurchase, setNotifPurchase] = useState(false)
  const [notifPayment, setNotifPayment] = useState(false)
  const [notifInvestment, setNotifInvestment] = useState(true)
  ...
  const saveEmail = () => {
    if (emailDraft) {
      setInvestorUser({ ...investorUser, email: emailDraft })
    }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-24">
      <div className="px-5 py-3">
        <button
          onClick={() => setScreen("investor-view")}
